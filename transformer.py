# Transformer code for Parts 1 and 2
...
class Attention(nn.Module):
    ...
    class Head(nn.Module):
        ...
        def __init__(self, block_size, n_embd, head_size, masked=False):
            super().__init__()
    ...
    def __init__(self, block_size, n_head, n_embd, masked=False):
        super().__init__()

        self.n_head = n_head
        head_size = n_embd // n_head
        self.heads = nn.ModuleList([Attention.Head(block_size, n_embd, head_size, masked) for _ in range(n_head)])
...
class Encoder(nn.Module):
    ...
    class Block(nn.Module):
        def __init__(self, block_size, n_head, n_embd):
            super().__init__()
            self.attention = Attention(block_size, n_head, n_embd)
    ...
    def __init__(self, vocab_size, n_embd, block_size, n_head, n_layer, device):
        ...
        self.blocks = nn.ModuleList([Encoder.Block(block_size, n_head, n_embd) for _ in range(n_layer)])
...
class Decoder(nn.Module):
    ...
    class Block(nn.Module):
        def __init__(self, block_size, n_head, n_embd, n_hidden):
            super().__init__()
            self.attention = Attention(block_size, n_head, n_embd, masked=True)
            self.ff = nn.Sequential(
    ...
    def __init__(self, vocab_size, n_embd, block_size, n_head, n_layer, n_hidden, device):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([Decoder.Block(block_size, n_head, n_embd, n_hidden) for _ in range(n_layer)])
